...
public class AxeKillsDatabase {
    ...
    public AxeKillsDatabase(String path) throws SQLException {
        ...
        try (Statement statement = connection.createStatement()) {
            ...
            try {
                statement.execute("ALTER TABLE Axekills ADD COLUMN deaths INTEGER DEFAULT 0;");
    ...
    public void addKill(UUID playerUUID, String timestamp) {
        if (killExists(playerUUID)) {
            ...
            try (PreparedStatement preparedStatement = connection.prepareStatement("INSERT INTO Axekills (player_uuid, kills, deaths, timestamp) VALUES (?, 1, 0, ?)")) {
    ...
    public void addDeath(UUID playerUUID, String timestamp) {
        if (killExists(playerUUID)) {
            ...
            try (PreparedStatement preparedStatement = connection.prepareStatement("UPDATE Axekills SET deaths = ?, timestamp = ? WHERE player_uuid = ?")) {
            ...
            try (PreparedStatement preparedStatement = connection.prepareStatement("INSERT INTO Axekills (player_uuid, kills, deaths, timestamp) VALUES (?, 0, 1, ?)")) {
                preparedStatement.setString(1, playerUUID.toString());
                preparedStatement.setString(2, timestamp);
                preparedStatement.executeUpdate();
            } catch (SQLException e) {
    ...
    public int getDeaths(UUID playerUUID) {
        try (PreparedStatement preparedStatement = connection.prepareStatement("SELECT deaths FROM Axekills WHERE player_uuid = ?")) {
            preparedStatement.setString(1, playerUUID.toString());
            ResultSet resultSet = preparedStatement.executeQuery();
            if (resultSet.next()) {
